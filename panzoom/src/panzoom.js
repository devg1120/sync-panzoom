export const panzoom = (selector, options={}) => {
	...
	function set_sync_element_closure(clo) {
		sync_element_closure.push(clo);
	};
	...
	function do_move(elem, deltaX, deltaY) {
		...
		//  sync_elements[0]("sync", [deltaX, deltaY]);
		//}

		sync_element_closure.forEach(element => {
		  //element("sync", [lastPosX, lastPosY]);
		  element("sync", [deltaX, deltaY]);
                });

	}

	function __sync_do_move(LastPosX, LastPosY) {

		let elem = self_element;
	...
	function sync_do_move(deltaX, deltaY) {
		let elem = self_element;
	...
	function sync_handle_pointerdown(e) {
		//if(e.target !== e.currentTarget) return;
		//e.preventDefault();
		//e.stopPropagation();		
	...
	return (cmd, param) => {
             if (cmd == 'set') {
		set_sync_element_closure(param[0]);
             } else if (cmd == 'start') {
                //sync_do_move(self_element, param[0], param[1]);
                sync_handle_pointerdown( param[0]);
             } else if (cmd == 'sync') {
                //sync_do_move(self_element, param[0], param[1]);
                sync_do_move( param[0], param[1]);
	     }
